...
// Writes a set-cookie-string based on the standard definded in RFC6265 S4.1.1. //后添加的 修改
function writeCookieString(obj) {
  return Object.keys(obj).reduce(function(str, name) {
    var encodedName  = replaceSpecialChar(encodeCookieComponent(name));
    var encodedValue = replaceSpecialChar(encodeCookieComponent(obj[name]));
    str += (str ? '; ' : '') + encodedName + '=' + encodedValue.replace(/%25/g,'%');
    return str;
  }, '');
}
