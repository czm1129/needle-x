...
user_agent = 'Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 5.1; Trident/4.0; InfoPath.2; .NET CLR 2.0.50727)';
...
//后添加的
function show_debug(count, method, uri, config, post_data){
   if (config.debug){
	    console.log(clc.red.bold.bgWhite.underline("dubug_____________________________________________start"));
		console.log(clc.greenBright(method));
		console.log(clc.greenBright(uri));
		console.log("\r");
		var hopt = config.headers;
		for (var o in hopt){
			 console.log(clc.greenBright(o+":"+hopt[o]));
		}
		if (method.toUpperCase() == 'POST'){
			console.log("\r\n");
			console.log(clc.greenBright(post_data));
		}
		console.log(clc.red.bgWhite.underline("dubug_________________________________________________end"));
   }
}
//后添加的end
...
Needle.prototype.start = function() {
  ...
  config.debug = options.debug || false;   //后添加的
  if (data) {
    ...
    if (options.multipart) { // boss says we do multipart. so we do it.
      ...
      multipart.build(data, boundary, function(err, parts) {
        ...
        self.send_request(1, method, uri, config, parts, out, callback); //后添加的
      });
...
Needle.prototype.send_request = function(count, method, uri, config, post_data, out, callback) {
	show_debug(count, method, uri, config, post_data); //后添加的
